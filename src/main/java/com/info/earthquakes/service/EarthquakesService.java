package com.info.earthquakes.service;

import com.info.earthquakes.dto.FeatureCollection;
import com.info.earthquakes.exception.BusinessException;
...
public class EarthquakesService {
    ...
    static String countryBoundingBoxesJson =
    ...
    public FeatureCollection getEarthquakesWithPastdayAndCountrycode(Long pastday, String countrycode){

        Map<String, String[]> countryCodeToBoundingBox = getCountryBoundingMap();

        String countryCode = countrycode;
        // same query but for given country code

        LocalDate endtime =LocalDate.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        String query3 = "https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson" +
                "&starttime="+endtime.minusDays(pastday).format(formatter) +
                "&endtime="+endtime.format(formatter) +
                "&minlatitude=" + countryCodeToBoundingBox.get(countryCode)[1] +
                "&maxlatitude=" + countryCodeToBoundingBox.get(countryCode)[3] +
                "&minlongitude=" + countryCodeToBoundingBox.get(countryCode)[0] +
                "&maxlongitude=" + countryCodeToBoundingBox.get(countryCode)[2];


        FeatureCollection response = (FeatureCollection) getDataFromUrl(query3,FeatureCollection.class);
        if(response.getFeatures().isEmpty()){
            throw new BusinessException("No Earthquakes were recorded past "+ pastday +" days");
        }
        return response;
    }

    private Object getDataFromUrl(String url,Class responseType) {
        RestTemplate restTemplate = new RestTemplate();

        ResponseEntity<Object> responseUsers = restTemplate.exchange(
                url, HttpMethod.GET, new HttpEntity<>("parameters"), responseType);
        Object response = responseUsers.getBody();
        return response;
    }

    public static Map<String, String[]> getCountryBoundingMap() {
        Map<String, String[]> countryCodeToBoundingBox = new HashMap<>();
        String[] lines = countryBoundingBoxesJson.split("\n");
        for (String line : lines) {
            String[] words = line.split(":");
            String countryCode = cleanText(words[0]);
            String[] secondPart = words[1].split(",");
            for (int i = 0; i < secondPart.length; i++) {
                secondPart[i] = cleanText(secondPart[i]);
            }
            countryCodeToBoundingBox.put(countryCode, Arrays.stream(secondPart).skip(1).toArray(String[]::new));
        }
        return countryCodeToBoundingBox;
    }
